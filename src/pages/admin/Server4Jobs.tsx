...
function useServer4Queue() {
  return useQuery({
    queryKey: ["server4-queue"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("kannada_queue_items" as any)
        .select("*")
        .eq("server_ip", SERVER_4)
...
export default function Server4Jobs() {
  ...
  useEffect(() => {
    const ch = supabase
      .channel("server4-queue-rt")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "kannada_queue_items", filter: `server_ip=eq.${SERVER_4}` },
        () => {
          qc.invalidateQueries({ queryKey: ["server4-queue"] });
          qc.invalidateQueries({ queryKey: ["server4-jobs-scan"] });
        },
      )
      .subscribe();
  ...
  // Block jobs currently in flight or already completed in queue history
  const blockedIds = useMemo(() => {
    const s = new Set<string>();
    (queue.data ?? []).forEach((q) => {
      if (q.status === "queued" || q.status === "processing" || q.status === "completed") {
  ...
  async function enqueue(rows: ScanRow[], mode: "single" | "selected" | "all") {
    ...
    if (!eligible.length) {
      toast({ title: "Nothing to enqueue", description: "All selected jobs are already queued/processing/completed." });
      return;
    }
    setBusy(true);
    try {
      ...
      toast({ title: `Enqueued ${eligible.length} job(s) on .4` });
      setSelected({});
      qc.invalidateQueries({ queryKey: ["server4-queue"] });
      qc.invalidateQueries({ queryKey: ["server4-jobs-scan"] });
      kickWorker();
  ...
  async function stopAllProcessing() {
    if (!confirm("Cancel ALL queued & processing items on .4? In-flight GPU work may keep running until it finishes, but nothing new will be picked up.")) return;
    setBusy(true);
    try {
      ...
      if (error) throw error;
      toast({ title: `Stopped ${count ?? 0} item(s) on .4` });
      qc.invalidateQueries({ queryKey: ["server4-queue"] });
      qc.invalidateQueries({ queryKey: ["server4-jobs-scan"] });
    } catch (e: any) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Server .4 Jobs — Kannada Queue</h1>
        <p className="text-sm text-muted-foreground">
          All jobs previously generated on <code>{SERVER_4}</code>. Select one or many
          to enqueue Kannada generation on the same server via the multilang avatar endpoint.
        </p>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="outline">Total on .4: {scan.data?.length ?? 0}</Badge>
        <Badge variant="secondary">Shown: {visibleJobs.length}/{filteredJobs.length}</Badge>
        <Badge className="bg-emerald-500">
          Already queued/done: {blockedIds.size}
        </Badge>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Queue on .4 ({queue.data?.length ?? 0})</CardTitle>
        </CardHeader>
        <CardContent>

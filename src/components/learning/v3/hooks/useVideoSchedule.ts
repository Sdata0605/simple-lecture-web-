...
import type { V3Section, V3VisualBeat } from '../types';
...
export function useVideoSchedule({ section, jobId, avatarVideoRef, getBlob }: UseVideoScheduleOpts) {
  ...
  useEffect(() => {
    ...
    // Pre-fetch remaining video blobs that weren't preloaded
    updatedBeats.forEach((b) => {
      if (b.type !== 'video' || b.blobUrl) return;
      console.log(`[VideoSchedule] Fetching non-preloaded video: ${b.src.slice(-60)}`);
      fetch(b.src)
        .then((r) => r.blob())
        .then((blob) => {
          ...
          console.log(`[VideoSchedule] Local blob created for: ${b.src.slice(-60)}`);
          setSchedule((prev) =>
            prev.map((e) => (e.src === b.src ? { ...e, blobUrl: url } : e))
          );
        })
        .catch(() => {
          console.warn(`[VideoSchedule] Failed to fetch: ${b.src.slice(-60)}`);
        });
    });

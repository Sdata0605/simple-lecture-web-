...
BEGIN
  ...
  query_tokens AS (
    ...
    FROM regexp_split_to_table(lower(regexp_replace(p_text, '[^a-z0-9 ]', ' ', 'g')), '[[:space:]]+') AS tok(token)
  ...
  candidate_tokens AS (
    ...
    FROM candidates c
    CROSS JOIN LATERAL regexp_split_to_table(lower(regexp_replace(c.candidate_question_text, '[^a-z0-9 ]', ' ', 'g')), '[[:space:]]+') AS tok(token)
    WHERE length(tok.token) >= 3
      AND NOT EXISTS (SELECT 1 FROM stopwords sw WHERE sw.word = tok.token)
  ),
  scored AS (

CREATE OR REPLACE FUNCTION public.scan_video_generation_coverage(p_subject_names text[] DEFAULT NULL::text[])
RETURNS TABLE(
  subject_name text,
  chapter_id uuid,
...
BEGIN
  ...
  SELECT
    ...
    a.chapter_id,
    a.chapter_number,
    a.chapter_title::text,
    a.topic_id,
    a.topic_number::text,
    a.topic_title::text,
    a.total_jobs,
    a.published_completed_jobs,
    l.status::text AS latest_status,
    l.job_id::text AS latest_job_id,
    l.external_job_id::text AS latest_external_job_id,
    l.server_ip::text AS latest_server_ip,
...
GRANT EXECUTE ON FUNCTION public.scan_video_generation_coverage(text[]) TO authenticated;

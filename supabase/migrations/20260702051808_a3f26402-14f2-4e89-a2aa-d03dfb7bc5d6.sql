...
BEGIN
  ...
  query_tokens AS (
    ...
    WHERE length(w) >= 3
      AND NOT EXISTS (SELECT 1 FROM stopwords sw WHERE sw.word = w)
  ),
  candidates AS (
    SELECT
  ...
  candidate_tokens AS (
    SELECT DISTINCT c.candidate_id, w AS token
    FROM candidates c
    CROSS JOIN LATERAL unnest(regexp_split_to_array(lower(regexp_replace(c.candidate_question_text, '[^a-z0-9 ]', ' ', 'g')), '\s+')) AS w
    WHERE length(w) >= 3
      AND NOT EXISTS (SELECT 1 FROM stopwords sw WHERE sw.word = w)
  ),
  scored AS (

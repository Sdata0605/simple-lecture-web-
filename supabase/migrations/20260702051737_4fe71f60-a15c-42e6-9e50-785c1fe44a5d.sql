...
BEGIN
  ...
  query_tokens AS (
    ...
    FROM unnest(regexp_split_to_array(lower(regexp_replace(p_text, '[^a-z0-9 ]', ' ', 'g')), '\s+')) AS w
    WHERE length(w) >= 3
      AND NOT EXISTS (SELECT 1 FROM stopwords s WHERE s.word = w)
  ),
  candidates AS (
    SELECT
      q.id,
  ...
  candidate_tokens AS (
    SELECT DISTINCT c.id, w AS token
    FROM candidates c
    CROSS JOIN LATERAL unnest(regexp_split_to_array(lower(regexp_replace(c.question_text, '[^a-z0-9 ]', ' ', 'g')), '\s+')) AS w
    WHERE length(w) >= 3
      AND NOT EXISTS (SELECT 1 FROM stopwords s WHERE s.word = w)
  ),
  scored AS (
    SELECT
      ...
      COALESCE((
        SELECT COUNT(*)::real FROM (
          SELECT token FROM candidate_tokens WHERE id = c.id
          INTERSECT
  ...
  SELECT
    ...
    fs.score,
    CASE
      WHEN fs.score >= 0.75 THEN 'near_duplicate'
      WHEN fs.score >= 0.45 THEN 'related'
      ELSE 'weak'
    END AS bucket
